...
export default function OmissionsHistory({omissionsf_parameter}){
    const [data, setData] = useState(omissionsf_parameter)
                                    ...
                                    &&(
                                        <div className={style.registerContainer} key={doc.id}>
                                            ...
                                            <div className={style.name}>
                                                <p className={style.pendingText}>{doc.user_id.first_name} {doc.user_id.last_name} {doc.user_id.second_last_name}</p>
                                            </div>

                                            <div className={style.timeLeft}>
                                                <p style={doc.omission_state== "Gestionado" ? {color: '#1D2958'}: doc.is_revised? {color: '#616161'}:{color: 'rgba(148, 2, 2, 0.7)'}}>{doc.omission_state == "Gestionado" ? "Gestionado" : doc.is_revised? "Visto":"Pendiente"}</p>
                                            </div>
                                            
                                            <div className={style.goTo}>
                                                <Link href={`/mipp/manage/omission/${doc.id}`}>
